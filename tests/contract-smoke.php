...
$load = static function ( string $name ) use ( $method ): array {
	$raw = json_decode( file_get_contents( TSE_APURACAO_DIR . 'tests/fixtures/' . $name ), true, 512, JSON_THROW_ON_ERROR );
...
if ( 'https://resultados.tse.jus.br/oficial/comum/config/ele-c.json' !== TSE_Discovery::config_url( 'oficial' ) ) {
	throw new RuntimeException( 'Official EA11 URL contract failed.' );
}
if ( 'https://resultados-sim.tse.jus.br/simulado/simulado2026/comum/config/ele-c.json' !== TSE_Discovery::config_url( 'simulado' ) ) {
	throw new RuntimeException( 'Simulation EA11 URL contract failed.' );
}
if ( ! str_ends_with( TSE_Discovery::candidates_url( 2026 ), '/consulta_cand_2026.zip' ) ) {
	throw new RuntimeException( 'Candidate package URL contract failed.' );
}
$url_method = new ReflectionMethod( TSE_Discovery::class, 'result_url' );
...
$scopes_method = new ReflectionMethod( TSE_Discovery::class, 'contest_scopes' );
